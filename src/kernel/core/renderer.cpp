...
void Renderer::ClearScreen(RGBColor color)
{
    for (unsigned int y = 0; y < currentFrameBuffer->Height; ++y)
    {
        for (unsigned int x = 0; x < currentFrameBuffer->Width; ++x)
        {
            DrawPixel(x, y, color);  // Assuming DrawPixel is a method that sets a pixel's color
        }
    }
}
