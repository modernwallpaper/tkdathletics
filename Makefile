...
BOOTLOADER_LD_LIBRARIES = -lefi -lgnuefi /usr/lib/gcc/x86_64-elf/14.2.0/libgcc.a -T $(DEPENDENCIES_DIR)/efi/gnuefi/elf_x86_64_efi.lds
...
KERNEL_CFLAGS = -ffreestanding -Wno-write-strings -fno-use-cxa-atexit -nostdlib -fno-builtin -fno-rtti -fno-exceptions -Iinclude/kernel/
...
createimg: $(DIST_DIR)/bootloader.efi startup.nsh $(DIST_DIR)/kernel.elf
	# Create a larger image (100MB in this case)
	dd if=/dev/zero of=$(DIST_DIR)/$(OSNAME).img bs=512 count=204800
	# Format the image with FAT16 filesystem using mkfs.fat
	mkfs.fat -F 16 -n COS $(DIST_DIR)/$(OSNAME).img
	# Create the necessary directories on the FAT filesystem
	mmd -i $(DIST_DIR)/$(OSNAME).img ::/EFI
	mmd -i $(DIST_DIR)/$(OSNAME).img ::/EFI/BOOT
	# Copy the bootloader, kernel, and other files
